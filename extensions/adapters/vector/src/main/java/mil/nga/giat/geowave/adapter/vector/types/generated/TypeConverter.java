...
public class TypeConverter
{
	 ...
	 * @throws IOException
	 */
	public byte[] serializeSingleFeatureCollection(
			final SingleFeatureCollection avroObject )
			throws IOException {
	 ...
	 * 
	 * @param features
	 *            Features to be serialized (must have at least one value)
	 * @param avroObjectToReuse
	 *            null or SingleFeatureCollection instance to be re-used. If
	 *            null a new instance will be allocated
	...
	public byte[] serializeSingleFeatureCollection(
			...
			SingleFeatureCollection avroObjectToReuse,
			Map<String, String> defaultClassifications,
			String defaultClassification )
			throws IOException {
		if (features == null || features.size() == 0) {
			throw new IOException(
					"Collection can not be null and must have at least one value");
		}
		...
		return serializeSingleFeatureCollection(
				itr,
				avroObjectToReuse,
	...
	public byte[] serializeSingleFeatureCollection(
		...
		for (AttributeDescriptor attr : sft.getAttributeDescriptors()) {
			...
			if (classification == null) {
				throw new IOException(
						"No default classification was provided, and no classification for: '" + attr.getLocalName() + "' was provided");
			}
	...
	/***
	 * @param avroData
	 *            serialized bytes of SingleFeatureCollection
	 * @param avroObjectToReuse
	 *            null or SingleFeatureCollection instance to be re-used. If
	 *            null a new object will be allocated.
	 * @return instance of SingleFeatureCollection with values parsed from
	 *         avroData
	 * @throws IOException
	 */
	private SingleFeatureCollection deserializeSFC(
			final byte[] avroData,
			SingleFeatureCollection avroObjectToReuse )
			throws IOException {
	 ...
	 * 
	 * @param avroData
	 *            serialized bytes of a SingleFeaturecollection
	 * @return Collection of GeoTools SimpleFeature instances.
	 * @throws IOException
	 * @throws ClassNotFoundException
	 * @throws ParseException
	 */
	public List<SimpleFeature> deserializeSingleFeatureCollection(
			final byte[] avroData )
			throws IOException,
			ClassNotFoundException,
			ParseException {

		SingleFeatureCollection sfc = deserializeSFC(
				avroData,
				null);
		...
		for (AttributeValues av : sfc.getValues()) {
			...
			Preconditions.checkArgument(featureNames.size() == av.getValues().size()); // null
																						// values
																						// should
																						// still
																						// take
